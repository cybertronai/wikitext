"""DiffusionBlocks AR — joint-sequence self-attention debug.

NOT a submission. No CharModel, no leaderboard, no wall-clock cap.

Fork of research/debug/diffusionblocks_ar_geneval/run.py. Only difference:
the attention inside each DBlock is *standard* causal self-attention over
the concatenated [x_emb || z] sequence of length 2T — single QKV
projection, single SDPA call with a plain causal mask, RoPE positions
flat over 0..2T-1. The z half is sliced out at the block exit.

The geneval variant used a custom mask + logical RoPE positions (x and
z shared positions 0..T-1, x→z attention was explicitly forbidden).
This file is the simplest possible "self-attention over the joint
sequence" baseline, to check whether the extra mask shaping mattered.

Usage (on a CUDA host with torch + transformers installed):

    python research/debug/diffusionblocks_ar_selfattn/run.py \\
        --data-dir /data \\
        --steps 12000 --batch-size 32 --max-len 1024 \\
        --n-samples 50 --seq-len 256 --n-steps 50 \\
        --out research/debug/diffusionblocks_ar_selfattn/

Writes:
  - result.json (per spec, with "architecture": "joint_sequence_selfattn")
...
class SelfLayer(nn.Module):
    """Standard causal self-attention over the joint sequence [x_emb || z].

    Single QKV projection over all N = T_x + T_z positions, single SDPA
    call with `is_causal=True` (flat lower-triangular mask), RoPE applied
    to the full sequence with positions 0..N-1. No custom masking, no
    logical-position trickery — the plainest possible self-attention.
    """
...
def train_model(text, *, n_steps, batch_size, max_len, num_layers, num_blocks,
    ...
    print(f"[selfattn] σ_data = {sd:.4f}", flush=True)
    print(f"[selfattn] block boundaries: {model.block_sigmas.tolist()}", flush=True)
    ...
    print(f"[selfattn] {n_params/1e6:.2f}M params  steps={n_steps}  bs={batch_size}  T={max_len}", flush=True)
    ...
    for step in range(n_steps):
        ...
        if log_every and (step % log_every == 0 or step == n_steps - 1):
            ...
            print(f"[selfattn] step {step:5d}/{n_steps}  b={b} σ={sigma_t:.3f}  "
...
def generate(model, n_samples=50, seq_len=256, device="cuda", n_steps=50, rho=7.0,
    ...
    logits = z_n @ E.float().t()
    ids = logits.argmax(-1).cpu().numpy().astype(np.uint8)
...
def gpt2_perplexity(texts, device="cuda", batch_size=4):
    ...
    for t in texts:
        ...
        per.append(math.exp(min(loss, 30.0)))
...
def main():
    ...
    ap.add_argument("--out", default="research/debug/diffusionblocks_ar_selfattn")
    ...
    print(f"[selfattn] loading WikiText-103 from {args.data_dir}", flush=True)
    train_text = _read_split(args.data_dir, "train")
    val_text = _read_split(args.data_dir, "valid")
    print(f"[selfattn] train chars={len(train_text):,}  val chars={len(val_text):,}", flush=True)
    ...
    print(f"[selfattn] generating {args.n_samples} samples × {args.seq_len} bytes "
    ...
    print(f"[selfattn] wrote {out_dir/'result.json'}", flush=True)
    print(f"[selfattn] PPL gen = {ppl_gen['mean']:.1f} ± {ppl_gen['std']:.1f}  "
