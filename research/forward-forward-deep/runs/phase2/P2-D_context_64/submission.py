"""P2-D — Context K=64 (FF investigation diagnostic).

Pass-2's submission with two changes: K 24 -> 64 (input dim 6400 ->
16640), and N_STEPS 14000 -> 10000 (larger input increases per-step
cost). Rule, schedule, negatives, width, readout — identical to pass-2.

Diagnostic value: slope of val char-acc vs context informs whether
Phase 7 should compound K=128 with backbone choices.

Spec: .survey/ff_runs/phase2/P2-D_context_64/design.md
"""
from __future__ import annotations

__author__ = "@survey-ff-p2d"
...
VOCAB = 256
K = 64                              # was 24
INPUT_DIM = (K + 1) * VOCAB         # 16640
N_LAYERS = 5
WIDTH = 384
THETA = 2.0
BATCH = 256
N_STEPS = 10000                     # was 14000
LR = 3e-4
BETAS = (0.9, 0.99)

HARD_NEG_EVERY = 350                # scaled from 500/14000 -> 350/10000
...
def _train_ff(train_text: str, device: torch.device, seed: int) -> tuple[FFStack, Tensor]:
    ...
    print(f"[p2d] {n_params/1e6:.2f}M params  layers={N_LAYERS} width={WIDTH} K={K} "
    ...
    for step in range(N_STEPS):
        if step > 0 and step % HARD_NEG_EVERY == 0:
            ...
            model.train()
            print(f"[p2d] step {step:5d}  hard-neg ridge refit ({HARD_NEG_REFIT_N} samples, "
        ...
        if step % log_every == 0 or step == N_STEPS - 1:
            ...
            print(f"[p2d] step {step:5d}/{N_STEPS}  loss(sum) {total:.4f}  "
