...
- Upper tier H: 4-layer d=192 modded-nanogpt transformer. Trained ONLY on
...
# Surprise threshold. D1 used tau=0.1 with a 2L/d=128 transformer's
# softmax probability. Our surprise proxy uses order-4 n-gram MLE
# (more concentrated than transformer softmax → equivalent surprise
# rate at higher tau). Empirically on synthetic-ish text:
#   transformer p_s(0.1) ≈ ngram-order4 p_s(0.3).
# We target p_s ~ 0.25 on WikiText (matches D1 spirit).
TAU = 0.30

# Upper tier H (NN). Smaller than alpha_06's d=256 since trained on
# subset; 800 steps (vs 1200).
H_MODEL_DIM = 192
H_NUM_LAYERS = 4
H_HEAD_DIM = 64
H_MAX_LEN = 512
...
# Inference mix. Lower than alpha_06 alpha=0.60 because the NN is
# only trained on a hard subset; let n-gram dominate easy positions.
ALPHA = 0.50
...
class ChunkerPhase1CharModel(CharModel):
    ...
    def predict(self) -> dict[str, float]:
        ...
        # Surprise-gated mix: if KN is very confident (p_kn.max >= 1-tau),
        # rely more on KN. Else mix in NN.
        p_kn_max = p_kn.max()
        if p_kn_max >= (1.0 - self.tau):
            # Easy byte: KN dominates. Small NN contribution for robustness.
            p_mix = 0.85 * p_kn + 0.15 * p_nn
        else:
            # Hard byte: NN (trained on surprises) takes lead.
