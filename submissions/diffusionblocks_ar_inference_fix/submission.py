"""DiffusionBlocks AR for byte-level WikiText-103 — paper-faithful inference.

Variant of submissions/diffusionblocks_ar/ that restores Algorithm 2
(paper.md:371-381) at inference: an N-step EDM (Karras ρ=7) schedule
walked from σ_max → σ_min, with each step *routed* to the block whose
[σ_b, σ_{b+1}) range contains the current σ. The original submission
collapsed N → B with σ pinned to block boundaries; this version takes
N ≫ B (default 50, configurable via DBLOCKS_N_STEPS).

The σ-independent KV-cache invariant is preserved (K, V are still
projected from x_emb only), so observe() is unchanged and the extra
Euler steps only inflate predict() cost. Per-byte cost goes from
B·(L/B) = L to N·(L/B) layer-forwards inside predict(). Training is
unchanged.
...
def _train(text: str, cfg: TrainConfig, device: torch.device) -> DBlocksAR:
    ...
    for step in range(cfg.n_steps):
        ...
        with _maybe_autocast(device):
            ...
            logits = model.logit_scale * (pred_y.float() @ E.float().t())
...
class DBlocksCharModel(CharModel):
    """N-step EDM Euler inference over the per-block KV-cache.
    ...
    - observe(c): for each byte of c, projects K/V from x_emb and appends
      to every block's cache (L layer-forwards total per byte).
    - predict(): EDM (Karras ρ=7) σ schedule from σ_max → σ_min in N
      steps. Each step is routed to the block b such that the current σ
      lies in [σ_b, σ_{b+1}); only that block's forward_infer runs.
      Final argmax over (z @ E_out.T) commits a byte. (Paper Algorithm 2,
      paper.md:371-381.)

    N is read from DBLOCKS_N_STEPS (default 50, matching paper §D.2 LM).
    """

    def __init__(self, model: DBlocksAR, device: torch.device | None = None,
                 n_steps: int | None = None, rho: float = 7.0):
        self.model = model
        ...
        if n_steps is None:
            n_steps = int(os.environ.get("DBLOCKS_N_STEPS", "50"))
        self.N = max(n_steps, self.B)
        self.rho = rho
        sigma_min = boundaries[0]
        sigma_max = boundaries[-1]
        # Karras EDM schedule: σ_i = (σ_max^(1/ρ) + i/N · (σ_min^(1/ρ) -
        # σ_max^(1/ρ)))^ρ, i = 0..N. Length N+1; N Euler intervals.
        inv_rho = 1.0 / rho
        s_max_r = sigma_max ** inv_rho
        s_min_r = sigma_min ** inv_rho
        self.sigma_schedule = [
            (s_max_r + (i / self.N) * (s_min_r - s_max_r)) ** rho
            for i in range(self.N + 1)
        ]
        # Route each step to its block. For step i with σ = sigma_schedule[i],
        # find b such that σ ∈ [σ_b, σ_{b+1}). Highest noise → block B-1,
        # lowest noise → block 0. Clamp at the endpoints since the EDM
        # schedule may sit exactly on σ_max / σ_min.
        import bisect
        self.step_block: list[int] = []
        for sigma in self.sigma_schedule[:-1]:
            b = bisect.bisect_right(boundaries, sigma) - 1
            self.step_block.append(min(max(b, 0), self.B - 1))
    ...
    def predict(self) -> str:
        ...
        logits = self.model.logit_scale * (
            z.squeeze(0).squeeze(0).float() @ E.float().t()
        )  # [V]
