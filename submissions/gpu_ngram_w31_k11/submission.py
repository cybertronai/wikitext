"""GPU port of W3 (deep_backoff_kn): order-12 chained backoff + Kneser-Ney smoothing.

Paradigm WTX-W031. Worker tag worker-3.

Hypothesis: W3 (deep_backoff_kn) hits 0.7184 at order-14 / KN-smoothed
but builds entirely on CPU/numpy → L2 spirit flag. This port runs the
n-gram counting on GPU via torch.unique on dual-int64 packed keys,
removing the L2 ambiguity while preserving the algorithm.
