"""Hebbian Fast-Weight Block (Schlag-Schmidhuber delta rule, gradient-free).

Experiment 07. Architecture: 4-layer modded-nanogpt body + 1
HebbianFastWeightBlock at the last position (block index 4, i.e. 5 blocks
total).

The HebbianFastWeightBlock maintains a fast-weight matrix W (B, d, d)
updated by the Schlag-Schmidhuber 2021 delta rule:

    v_bar_t = W_{t-1} φ(k_t)                          # retrieve
    W_t     = W_{t-1} + β_t (v_t - v_bar_t) ⊗ φ(k_t)  # write residual

Gradient contract: W is a non-differentiable hidden state (updated under
torch.no_grad). Output o_t = W_{t-1} q_t propagates gradient back through
q_t and `proj` only. k, v, β projections affect dynamics through their
init values; they receive no gradient.

The scan is implemented in the chunkwise parallel form (Yang et al. 2024,
"Parallelizing Linear Transformers with the Delta Rule over Sequence
Length"): within each chunk of size 64, the per-step recurrence collapses
into one unit-lower triangular solve plus a handful of batched matmuls,
so 1024 inner sequential iterations become 16 outer iterations each
issuing O(1) GPU kernels. The fast-weight matrix W is carried across
chunks. Mathematically equivalent to the sequential loop for decay=1.0.

For streaming inference (`observe()`), one delta-rule write step is
applied per byte; the per-stream W is part of model state and is zeroed
on `reset()`.

Based on:
  submissions/modded_nanogpt/submission.py  (modded-nanogpt port)
"""
...
# ---------------------------------------------------------------------------
# Hebbian Fast-Weight Block (Schlag-Schmidhuber delta rule, gradient-free W)
# ---------------------------------------------------------------------------

class HebbianFastWeightBlock(nn.Module):
    """Single transformer-style block whose "attention" is replaced by a
    Schlag-Schmidhuber 2021 delta-rule fast-weight scan.

    Parameters (trainable, SGD): q, k, v, beta, proj, norm1, norm2.
    Hidden state (NOT trainable, no backprop): W: (B, d, d).

    Forward call (training): chunked causal scan over T. Within each
    chunk (chunk_size=64), we walk the recurrence step by step. Across
    chunks we carry the running W. W is updated under torch.no_grad —
    gradient flows back only through (norm1, q, proj, norm2, mlp).

    Streaming call (T=1, fw_state=W provided): one delta step against
    the carried W, returning the new W. No backprop required at eval.
    """
    def __init__(self, dim: int, chunk_size: int = 64, decay: float = 1.0):
        ...
        # The block also has a feed-forward part on the residual stream,
        # matching the Block layout (norm2 + mlp) so the substitution is
        # plug-compatible. The fast-weight scan replaces only the attn.
        self.norm2 = RMSNorm(dim)
        self.mlp = MLP(dim)

    def _scan_chunk(
        self,
        q_chunk: Tensor,        # (B, Tc, d)  -- fp32, normalized
        k_chunk: Tensor,        # (B, Tc, d)  -- fp32, normalized
        v_chunk: Tensor,        # (B, Tc, d)  -- fp32
        beta_chunk: Tensor,     # (B, Tc)     -- fp32
        W: Tensor,              # (B, d, d)   -- fp32, no_grad detached
    ) -> tuple[Tensor, Tensor]:
        """Chunkwise parallel form of the delta-rule scan.

        Replaces the per-step Python recurrence with O(1) bmm + triangular-solve
        ops per chunk (Yang et al. 2024, "Parallelizing Linear Transformers with
        the Delta Rule over Sequence Length"). For chunk inputs (Q, K, V) of
        shape (B, C, d), β of shape (B, C), and initial state W (B, d, d),
        define U ∈ R^{B×C×d} where row t = u_t^T = β_t (v_t - W_t k_t)^T. By
        unrolling W_t = W_0 + Σ_{s<t} u_s k_s^T,

            (I + β · strict_lower(K K^T)) U = β · (V - K W^T)
            O = Q W^T + strict_lower(Q K^T) U
            W_new = W + U^T K

        which is mathematically equivalent to the sequential loop for
        decay = 1.0. The 1024-step inner Python loop collapses to a handful of
        batched matmuls and one unit-lower triangular solve per chunk.
        """
        assert self.decay == 1.0, (
            "chunkwise parallel scan currently supports decay=1.0 only"
        )
        B, C, d = q_chunk.shape
        device = q_chunk.device
        dtype = q_chunk.dtype  # fp32 (caller upcasts q/k/v/β before this call)

        # Writes run gradient-free, matching the original `with torch.no_grad()`
        # contract: u_t and W carry no autograd graph.
        with torch.no_grad():
            ...
            # V_tilde[t] = V[t] - K[t] W^T : the residual against the chunk's
            # starting state, before any in-chunk writes.
            V_tilde = v_det - torch.bmm(k_det, W_det.transpose(-1, -2))

            # A[t,s] = β_t (K[t] · K[s]) for s < t. Diagonal entries are
            # arbitrary because solve_triangular runs with unitriangular=True.
        ...
        # Reads: gradient flows only through q. K, W, U stay detached, matching
        # the sequential version where the writes ran under no_grad and only
        # the read o_t = W_{t-1} q_t carried gradient back through q.
    ...
    def _scan(
        ...
        v: Tensor,     # (B, T, d)
        beta: Tensor,  # (B, T)
        W0: Tensor,    # (B, d, d) initial fast-weight state (no grad)
    ) -> tuple[Tensor, Tensor]:
        """Chunked causal scan. W0 -> W_T; returns (outputs, W_T)."""
        B, T, d = q.shape
        outs: list[Tensor] = []
        W = W0
        cs = self.chunk_size
    ...
    def forward(
        ...
        offset: int = 0,  # accepted for signature compat; unused
    ) -> tuple[Tensor, Tensor]:
        """Apply the Hebbian fast-weight scan + MLP, residual style.

        Args:
            x: (B, T, d) hidden states.
            fw_state: optional running W: (B, d, d). If None, start with
                zeros. Returned so the caller can carry it across calls
                (used by streaming inference).
        ...
        # Sub-layer 1: fast-weight scan with pre-norm and residual.
        h = self.norm1(x)
        # L2-normalize q, k per Schlag (sum-of-keys denominator dropped).
        # All scan math in fp32 — bf16 outer-product accumulation in W
        # would lose precision over T=1024 writes.
        q = F.normalize(self.q(h).float(), dim=-1, eps=1e-6)
        k = F.normalize(self.k(h).float(), dim=-1, eps=1e-6)
        v = self.v(h).float()
        ...
        # Scan returns chunked outputs.
        out_fp32, W_new = self._scan(q, k, v, beta, W)
        out = out_fp32.type_as(x)
        x = x + self.proj(out)

        # Sub-layer 2: MLP with pre-norm and residual (matches Block).
        x = x + self.mlp(self.norm2(x))
        return x, W_new
...
class GPT(nn.Module):
    ...
    Total = 5 blocks. The standard self-attention blocks produce a KV
    cache per layer; the Hebbian block produces a fast-weight state W
    instead. Forward returns both — the streaming wrapper carries them
...
def _train_hebbian(
    ...
    # Optimizer split: standard 2-D weights get Muon; everything else (incl.
    # the Hebbian block's q/k/v/beta/proj 2-D weights) — actually, the FW
    # block's projections ARE 2-D weights and benefit from Muon too. So we
    # let Muon handle every 2-D ndim>=2 weight in `blocks` and `fw_block`.
    ...
    print(f"[hebbian_fw] {n_params/1e6:.2f}M params  cfg={cfg}")
    ...
    for step in range(cfg.n_steps):
        ...
        # During training the fast-weight state always starts fresh per
        # batch (each sampled window is independent, and W is per-batch
        # sample anyway). We discard W_new.
        if use_amp:
        ...
        if cfg.log_every and (step % cfg.log_every == 0 or step == cfg.n_steps - 1):
            ...
            print(
                f"[hebbian_fw] step {step:5d}/{cfg.n_steps}  "
                f"loss {loss.item():.4f}  "
...
class HebbianFWCharModel(CharModel):
    ...
    def reset(self) -> None:
        ...
        self._pos = 0
        # Seed with one zero byte to get a valid distribution for the
        # first predict() call.
