"""LWTA-k=4 NN (d=256, L=4) + W31 GPU order-12 KN n-gram at α=0.65.

Paradigm: combines the two best 2026-05-19 wins on top of the clean
W31 hybrid recipe:

* lwta_k4_plus_w31 at α=0.7: 12,102 J / 0.7332  (LWTA k=4 in MLP)
* alpha_065 at α=0.65:        15,307 J / 0.7387  (ReLU^2 MLP at α=0.65)

Both factors push +0.55pp / +0.08pp independently — stacking them on
the same hybrid is the natural follow-up. α=0.65 leans a bit more on
W31 (35% n-gram, 65% NN), which appears to be the right balance when
the NN is sparser (LWTA k=4 only keeps 25% of MLP activations).

Hypothesis: 11-13 kJ / 0.738+ acc. Modal A100-80GB PCIe, fully GPU-bound
(KN build + NN train both on GPU).

Build path:
  1. GPU phase A: W31 KN build (~75s on Modal A100).
  2. GPU phase B: train LWTA-k=4 NN with same Muon+AdamW recipe (~75s).
  3. Inference: NN softmax (GPU) blended with KN distribution
     (CPU side) at α=0.65.
"""
...
def lwta_k(x: Tensor, k: int) -> Tensor:
    """Local Winner-Take-All over groups of `k` along the last dim."""
    assert x.size(-1) % k == 0, f"hidden dim {x.size(-1)} not divisible by k={k}"
...
# ===========================================================================
# Part 1 — W31 GPU KN build (verbatim from clean_hybrid_w31).
# ===========================================================================
...
def build_w31_kn_tables(
    ...
    for new_k in range(max_order - 1, 0, -1):
        ...
        print(f"[lwta_k4_a065] ctx_len={new_k-1} ctxs={tbl['ctx_keys'].shape[0]:,} "
              f"{time.monotonic()-t0:.1f}s", flush=True)
        if new_k == 2:
    ...
    print(f"[lwta_k4_a065] KN build done: {time.monotonic()-t_total:.1f}s",
          flush=True)
...
# ===========================================================================
# Part 2 — modded-nanogpt NN with LWTA-k=2 in the MLP.
# ===========================================================================
...
class MLP(nn.Module):
    """MLP with LWTA-k=2 activation in place of ReLU^2."""
    def __init__(self, dim: int, k: int = LWTA_K):
        super().__init__()
        hdim = 4 * dim
        assert hdim % k == 0
    ...
    def forward(self, x):
        x = self.fc(x)
        x = lwta_k(x, self.k)  # was: x = x.relu().square()
        x = self.proj(x)
        return x
...
class TrainConfig:
    ...
    def __repr__(self):
        return (f"TrainConfig(d={self.model_dim} L={self.num_layers} "
                ...
                f"T={self.max_len} steps={self.n_steps} lwta_k={LWTA_K})")
...
def _train_modded(
    ...
    print(f"[lwta_k4_a065] NN {n_params/1e6:.2f}M params  cfg={cfg}")
    ...
    for step in range(cfg.n_steps):
        ...
        if cfg.log_every and (step % cfg.log_every == 0 or step == cfg.n_steps - 1):
            ...
            print(
                f"[lwta_k4_a065] NN step {step:5d}/{cfg.n_steps}  "
...
class LWTAW31CharModel(CharModel):
    """LWTA-k=4 NN + W31 GPU KN n-gram mixed at α=0.65."""
...
def train(train_text: str, valid_text: str | None = None) -> CharModel:
    ...
    if seed_env:
        ...
        print(f"[lwta_k4_a065] SEED={seed}")
    ...
    if is_smoke:
        ...
        print(f"[lwta_k4_a065] SMOKE mode (train={len(raw)} bytes)  "
    ...
    # Phase B: GPU NN train (LWTA-k=2 swap).
    model = _train_modded(train_bytes_u8, cfg, device)

    return LWTAW31CharModel(
