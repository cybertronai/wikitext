"""modded-nanogpt + isotropic Compositional Muon on attention OV pairs (MP=4).
...
def row_uniform_muon_update(
    ...
    update = muon_update(grad, momentum, mu=mu, nesterov=nesterov).float()
    if update.size(-2) > update.size(-1):
        row_norm = update.norm(dim=-1, keepdim=True).clamp(min=eps)
        target = update.norm(dim=(-2, -1), keepdim=True) / (update.size(-2) ** 0.5)
        update = update * (target / row_norm)
    return update.bfloat16()
...
class RowUniformMuon(torch.optim.Optimizer):
    """Muon with equalized row update norms for tall MLP input projections."""
    ...
    def step(self):  # type: ignore[override]
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if len(state) == 0:
                    state["momentum"] = torch.zeros_like(p)
                update = row_uniform_muon_update(p.grad, state["momentum"], mu=group["mu"])
                p.mul_(1 - group["lr"] * group["weight_decay"])
                p.add_(update, alpha=-group["lr"])
...
class TrainConfig:
    ...
    def __repr__(self):
        return (f"TrainConfig(d={self.model_dim} L={self.num_layers} "
                ...
                f"cm_mp={self.cm_mp} mlp_mp={self.mlp_mp})")
...
def _train_modded(
    ...
    # Compositional Muon gets attention V/O pairs. MLP expansion weights use
    # row-uniform Muon to test the tall-matrix leverage/dead-neuron hypothesis.
    ...
    mlp_fc = [block.mlp.fc.weight for block in model.blocks]
    mlp_fc_ids = {id(p) for p in mlp_fc}
    block_2d = [
        p for p in model.blocks.parameters()
        if p.ndim >= 2 and id(p) not in ov_ids and id(p) not in mlp_fc_ids
    ]
    ...
    optimizer4 = RowUniformMuon(
        mlp_fc,
        lr=cfg.muon_lr * cfg.mlp_mp,
        weight_decay=cfg.muon_wd,
    )
    ...
    print(f"[comp-muon] {n_params/1e6:.2f}M params  cfg={cfg}  "
          f"ov_pairs={len(ov_pairs)} mlp_row_uniform={len(mlp_fc)} "
          f"muon_params={len(block_2d)}")
...
def train(train_text: str, valid_text: str | None = None) -> CharModel:
    ...
    if os.environ.get("MLP_MP") is not None:
        kwargs["mlp_mp"] = float(os.environ["MLP_MP"])
