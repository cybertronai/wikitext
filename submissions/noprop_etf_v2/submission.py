"""NoProp-style local-denoising terminal stack v2 — ETF init + recon-loss fix.

Experiment 09 v2 (Variant A):
  ...
  * NoPropTerminalStack: T=10 chained denoising sub-blocks + label_embed
    (256 x 128) + linear readout(128, 256). Per Li-Teh-Pascanu
    arXiv 2503.24322 Eq. 8.
  * The body's hidden state is DETACHED before entering the NoProp stack —
    no NoProp gradient flows into the body.
...
v2 vs v1 changes (experiment_09_v2_noprop_orthogonal_init.md):
  * Fix A — orthogonal/ETF init for label_embed. For 256 classes in 128-d,
    use Lloyd-style ETF iteration to push pairwise cosines toward -1/(K-1).
  * Fix B — reconstruction loss target uses the denoiser-chain output
    z_0_pred (matched to inference), not the train-time noisy z_T_gt.
  * Fix C — KL term removed entirely.
  * Diagnostic — log label_embed pairwise off-diagonal cosine each
    log_every step (class-crowding probe).
...
References:
  ...
  Casazza, Kovacevic 2003                  (equiangular tight frames)
"""
...
def cosine_alpha_bar(T: int, s: float = 0.008, device=None, dtype=torch.float32) -> Tensor:
    ...
    # avoids near-singular SNR_0=0.
    return abar.clamp(0.02, 0.98)


def _equiangular_tight_frame(K: int, d: int, n_iter: int = 50, lr: float = 0.05) -> Tensor:
    """Lloyd-style iterative ETF construction.

    For K > d we cannot have K orthogonal vectors in R^d. The ETF is the
    optimal "as-orthogonal-as-possible" configuration: K unit vectors in
    R^d with pairwise inner product = -1/(K-1) (Welch-bound equality).

    For K=256, d=128 the Welch bound gives cos = -1/255 ~ -0.004.

    Returns: (K, d) tensor of unit-norm rows.
    """
    g = torch.randn(K, d)
    for _ in range(n_iter):
        g = F.normalize(g, dim=1)
        gram = g @ g.T - torch.eye(K)
        # Push pairwise inner products toward 0 (-1/(K-1) is so close to 0
        # for K=256 that this is effectively equivalent).
        g = g - lr * gram @ g
    return F.normalize(g, dim=1)


def _offdiag_cos(W: Tensor) -> float:
    """Mean absolute off-diagonal cosine of W (K, d) — class-crowding probe."""
    Wn = F.normalize(W, dim=1)
    gram = Wn @ Wn.T
    K = W.size(0)
    mask = ~torch.eye(K, dtype=torch.bool, device=W.device)
    return gram[mask].abs().mean().item()
...
class NoPropTerminalStack(nn.Module):
    ...
    def __init__(
        ...
        # Fix A — ETF init for label_embed (v2 change vs v1's normal(std=0.5)).
        with torch.no_grad():
            n_ortho = min(vocab_size, d_label)
            if n_ortho == vocab_size:
                # vocab_size <= d_label: fully orthogonal.
                W = torch.empty(vocab_size, d_label)
                nn.init.orthogonal_(W)
            else:
                # d_label < vocab_size: ETF gives near-equiangular unit vecs.
...
def _train_noprop(
    ...
    print(f"[noprop_v2] body={n_body/1e6:.2f}M  noprop={n_np/1e6:.2f}M  cfg={cfg}")
    print(f"[noprop_v2] alpha_bars = {[round(x, 4) for x in alpha_bars.tolist()]}")
    init_offdiag = _offdiag_cos(np_stack.label_embed.weight.data)
    init_norms = np_stack.label_embed.weight.data.norm(dim=-1)
    print(f"[noprop_v2] ETF init  label_norm_mean={init_norms.mean().item():.4f}  "
          f"label_norm_std={init_norms.std().item():.4f}  "
          f"label_offdiag_cos={init_offdiag:.4f}  (Welch bound for K=256,d=128 "
          f"is 1/255 ~ 0.0039)")
    ...
    for step in range(cfg.n_steps):
        ...
        # Fix B — reconstruction loss against the DENOISER-CHAIN output, not
        # the train-time noisy z_T. This matches the inference distribution:
        # inference starts from z ~ N(0, I) and rolls the chain to z_0_pred,
        # which is exactly what we score against y here.
        #
        # We roll the chain under no_grad through t = 1 .. T-1, then enable
        # grad for the final denoiser + readout. This way the gradient flows
        # into the modules whose inference-time behavior matters most (the
        # last denoiser and the readout). Earlier denoisers still receive
        # per-step SNR-weighted gradient via loss_np above.
        with torch.no_grad():
            z = torch.randn_like(y_emb)
            for t in range(1, cfg.noprop_T):
                u_pred_inner = np_stack.denoisers[t - 1](h_det, z, t / cfg.noprop_T)
                abar_t = alpha_bars[t]
                z = abar_t.sqrt() * u_pred_inner
        # Final denoiser step + readout WITH gradient.
        u_pred_final = np_stack.denoisers[cfg.noprop_T - 1](
            h_det, z, cfg.noprop_T / cfg.noprop_T
        )
        logits_np = np_stack.readout(u_pred_final)
        loss_recon = F.cross_entropy(logits_np.reshape(-1, 256), y.reshape(-1))

        # Fix C — KL term removed entirely (v1 had `loss_kl = 0.5 * (y_emb**2).mean()`
        # at weight 1e-3; with ETF init y_emb is unit-norm so the proxy is a
        # constant ~= 0.5 * 1.0 = 0.5 with zero gradient. No-op to remove.)
        total = loss_sgd + loss_np + loss_recon
        total.backward()
        for opt in optimizers:
            opt.step()
        ...
        if cfg.log_every and (step % cfg.log_every == 0 or step == cfg.n_steps - 1):
            ...
            with torch.no_grad():
                ...
                # should hit very high cos-sim with y_emb here.
                ...
                # Class-crowding probe (v2 diagnostic).
                offdiag = _offdiag_cos(np_stack.label_embed.weight.data)
            print(
                f"[noprop_v2] step {step:5d}/{cfg.n_steps}  "
...
def train(train_text: str, valid_text: str | None = None) -> CharModel:
    ...
    if valid_text is not None:
        ...
        print(
            f"[noprop_v2] diag val acc (1024-tok teacher-forced): "
