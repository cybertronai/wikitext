"""modded-nanogpt with one MLP block (block 2) replaced by a frozen-random-features MLP.

Experiment 06: Frozen Random-Features Layer Inside Gradient-Trained Transformer.
The fc projection (d -> 4d) in block 2's MLP is replaced by a frozen random
buffer; the proj (4d -> d) remains trainable. All other layers are unchanged
from the modded_nanogpt baseline.
...
class FrozenRFMLP(nn.Module):
    """MLP whose expansion projection (d -> 4d) is a frozen random buffer.

    Experiment 06: tests whether the fc weights in a single transformer MLP
    block carry task-adapted features or whether a random expansion suffices,
    provided the 4d -> d collapse is still trainable so SGD can pick which
    random features matter. Halves this block's trainable MLP params and skips
    its fc weight-grad outer product in backward.
    """
    def __init__(self, dim: int):
        super().__init__()
        hdim = 4 * dim
        ...
        # Bias of the frozen expansion is kept zero and frozen as well, so the
        # frozen direction is purely random.
        self.register_buffer("b_frozen", torch.zeros(hdim))
        self.proj = Linear(hdim, dim)

    def forward(self, x: Tensor) -> Tensor:
        W = self.W_frozen.type_as(x)
        b = self.b_frozen.type_as(x)
        h = F.linear(x, W, b)
        h = h.relu().square()
        return self.proj(h)
...
class Block(nn.Module):
    def __init__(self, dim: int, head_dim: int, frozen_mlp: bool = False):
        super().__init__()
        self.attn = CausalSelfAttention(dim, head_dim=head_dim)
        self.mlp = FrozenRFMLP(dim) if frozen_mlp else MLP(dim)
...
class GPT(nn.Module):
    def __init__(
        ...
        head_dim: int = 64,
        max_len: int = 1024,
        frozen_mlp_block: int | None = 2,
    ):
        super().__init__()
        self.vocab_size = vocab_size
        self.max_len = max_len
        self.frozen_mlp_block = frozen_mlp_block
...
def _train_modded(
    ...
    # Muon gets the 2-D block weights.
    ...
    print(
        f"[rf_mlp_b2] params {n_params/1e6:.2f}M trainable "
        f"{n_trainable/1e6:.2f}M buffers {n_buffers/1e6:.2f}M  "
        f"frozen_mlp_block={model.frozen_mlp_block}  cfg={cfg}"
    )
    ...
    for step in range(cfg.n_steps):
        ...
        if cfg.log_every and (step % cfg.log_every == 0 or step == cfg.n_steps - 1):
            ...
            print(
                f"[modded] step {step:5d}/{cfg.n_steps}  "
                f"loss {loss.item():.4f}  "
